...
void outputFile(char output_file[], int* reg_pointer){
	FILE *write_ptr;
    write_ptr = fopen(output_file,"wb");  // w for write, b for binary
    for(int i = 0; i < 32; i++){
        fwrite(reg_pointer,sizeof(int),1,write_ptr);
        reg_pointer++;
    }
    if (!write_ptr) {
...
int main() {
	...
	char file_path[80] = "loop";
    char input_file[80];
    strcpy(input_file, file_path);
    strcat(input_file, ".bin");
	...
	int* reg_pointer = &reg[0];

	FILE* ptr_myfile;

	ptr_myfile = fopen(input_file, "rb");
	if (!ptr_myfile) {
		printf("Unable to open file!");
		return 1;
	}
	...
	while (fread(memory_pointer, sizeof(int), 1, ptr_myfile)) {
		dec2bin(*memory_pointer); //prints the binary instructions
		printf("\n");
		word_counter++;
		memory_pointer++;
	}
	...
	while (pc < word_counter) {
		...
		case 0x73: // e-call
			...
			if (reg[10] == 10) {
				...
				printReg(reg);
				outputFile(output_file, reg_pointer);
		...
		case 0x3: // load 0000011
			...
			case 0b100://LBU
				...
				case 3:
					load_byte = memory[word_address] & 0xFF000000;
					load_byte = load_byte >> 24;
					load_byte = load_byte & 0x000000FF; // clear any bits that indicate sign
					// there may be a one in the most significant bit
                    reg[rd] = load_byte;
                    break;
				}
				break;
			...
			case 0b101://LHU
				...
				case 2:
					load_half = memory[word_address] & 0xFFFF0000;
					load_half = load_half >> 16;
					load_half = load_half & 0x0000FFFF; // clear any bits that indicate sign
                    // there may be a one in the most significant bit
					reg[rd] = load_half;
					break;
				}
		...
		default:
			printf("Op code %d not yet implemented \n", opcode);
		}


		//printReg(reg); // takes a lot of time
	}
	printReg(reg);
